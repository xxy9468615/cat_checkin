...
def main():
    ...
    login=h.request("GET",f"{base}/api/v2/users/login-info?device=desktop&t={nowms()}",headers=headers,timeout=20).text
    ...
    st=h.request("GET",f"{base}/api/v2/home/paulsigns/user?device=desktop&t={nowms()}",headers=headers,timeout=20).text
    signed=find(r'"signed":\s*(true|false)',st,"false"); days=find(r'"days":\s*(\d+)',st,"?")
    sign_points="0"
    if signed != "true":
        sr=h.request("POST",f"{base}/api/v2/home/paulsigns/sign?device=desktop",headers=headers,json_data={},timeout=20).text
        sign_points=find(r'"increased":\s*(\d+)',sr,"0")
        # 签到 POST 的响应不校验会假成功（网络错误/业务失败时 increased 缺失）：
        # 复查签到状态，仍未签上则显式失败
        st2=h.request("GET",f"{base}/api/v2/home/paulsigns/user?device=desktop&t={nowms()}",headers=headers,timeout=20).text
    ...
    try:
        if uid and uuid and time.monotonic()<=browse_deadline:
            own_ok=h.request("GET",f"{base}/pro/user-center?uid={uid}&uuid={uuid}&do=thread",headers=headers,timeout=15).code==200
        if time.monotonic()<=browse_deadline:
            threads=h.request("GET",f"{base}/api/v2/forum/thread/list?device=desktop&channel_slug=home-square-all&limit=10&offset=10&sort_type=latest_posts",headers=headers,timeout=15).text
            ...
            for auid, auuid in list(zip(uids,uuids))[:10]:
                ...
                try:
                    if h.request("GET",f"{base}/pro/user-center?uid={auid}&uuid={auuid}",headers=headers,timeout=15).code==200: other_ok+=1
                except Exception:
                    pass
    except Exception:
        pass
    final=h.request("GET",f"{base}/api/v2/users/login-info?device=desktop&t={nowms()}",headers=headers,timeout=20).text
